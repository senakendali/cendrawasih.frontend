       ...
       <div id="tournament-bracket">
        ...
        <div v-if="bracketType === 2" class="match-bracket">
          ...
          >
            ...
            <div class="player-1">
              <div class="p-name">
                {{ game && game.player1 ? (game.player1.name || '-') : '-' }}
              </div>
              <div class="p-contingent" v-if="game && game.player1 && game.player1.contingent">
            ...
            <!-- Player 2 -->
            <div class="player-2">
              <template v-if="!game || !game.player2 || game.player2.name === 'BYE'">
                TBD
              </template>
              <template v-else>
        ...
        <div
          v-else-if="rounds.length === 1 && rounds[0] && rounds[0].games && rounds[0].games.length === 1"
          class="final-only"
        >
          <div class="match-container">
            ...
            <div class="player-1">
              <div class="p-name">
                {{ (rounds[0].games[0].player1 && rounds[0].games[0].player1.name) ? rounds[0].games[0].player1.name : '-' }}
              </div>
            ...
            <div class="player-2">
              ...
              <template v-else>
                <div class="p-name">{{ rounds[0].games[0].player2.name }}</div>
                <div class="p-contingent" v-if="rounds[0].games[0].player2.contingent">
            ...
            <template v-slot:player="{ player }">
              <div
                ...
                :title="
                  ...
                  (
                    player && player.contingent &&
                    player.name && player.name.toUpperCase() !== 'BYE'
                      ? (' • ' + player.contingent)
                      : ''
                  )
                "
              >
                <div class="p-name">
                  {{ player && player.name ? player.name : 'TBD' }}
                </div>
                <div
                  class="p-contingent"
                  v-if="
...
export default {
  ...
  methods: {
    ...
    _normPlayer(raw, roundNum) {
      if (!raw) return { name: '', contingent: '', round: roundNum };
      if (typeof raw === 'string') {
        return { name: raw, contingent: '', round: roundNum };
      }
      // fallback: contingent bisa datang dg key berbeda (school/team_name/contingent)
      const name = (raw.name ?? '').toString();
      const contingent = (
        raw.contingent ?? raw.school ?? raw.team_name ?? ''
      ).toString();
      return { name, contingent, round: Number(raw.round ?? roundNum) };
    },
    stopAutoGenerate() {
    ...
    async fetchRounds() {
      this.loading = true;
      try {
        const { data } = await axios.get(`/pools/${this.poolId}/matches`);

        // pastikan number
        this.bracketType = Number(data.match_chart ?? 1);

        const srcRounds = Array.isArray(data.rounds) ? data.rounds : [];
        this.rounds = srcRounds.map((r, idx) => {
          const roundNum = Number(r.round ?? (idx + 1));
          const rows = Array.isArray(r.games) ? r.games
                    : Array.isArray(r.matches) ? r.matches
                    : [];
          const games = rows.map(g => {
            const p1 = g.player1 ?? g.blue ?? null;
            const p2 = g.player2 ?? g.red  ?? null;
            return {
              round: roundNum,
              player1: this._normPlayer(p1, roundNum),
              player2: this._normPlayer(p2, roundNum),
            };
          });
          return { round: roundNum, games };
        });

        // ⬅️ INI YANG KEMARIN KETINGGALAN
        this.games = (this.bracketType === 2 && this.rounds[0] && this.rounds[0].games)
          ? this.rounds[0].games
          : [];
