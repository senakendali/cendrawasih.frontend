<template>
  <div class="dashboard-container">
    ...
    <div class="mb-2 page-title">
      <i class="bi bi-file-earmark-text"></i> Match Clasification List
    </div>
    ...
    <div class="mb-2 d-flex justify-content-end align-items-center">
      ...
      <!-- Create Menu Button -->
      <router-link to="/admin/match-clasification/create" class="button button-primary">
    ...
    <table class="table mt-4">
      ...
      <tbody v-if="classes.length > 0">
        ...
        <tr v-for="(class_category, index) in classes" :key="class_category.id">
          ...
          <td>{{ class_category.tournament.name }}</td>
          <td>{{ class_category.match_category.name }}</td>
          ...
          <td class="action-column">
            <div class="btn-group" role="group">
              ...
              <ul class="dropdown-menu">
                <li>
                  <a class="dropdown-item" href="#" @click="EditClasification(class_category.id)">
      ...
      <tbody v-else>
        <tr>
          <td colspan="6" class="text-center">No class found.</td>
        </tr>
      </tbody>
...
export default {
  ...
  methods: {
    async loadClasses(page = 1) {
      this.loading = true;
      try {
        const response = await axios.get("/match-clasifications", {
          headers: {
    ...
    async EditClasification(id) {
      this.$router.push({ name: "EditClasification", params: { id } });
    },
