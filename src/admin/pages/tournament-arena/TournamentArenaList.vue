<template>
  <div class="dashboard-container">
    ...
    <div class="mb-2 page-title">
      <i class="bi bi-file-earmark-text"></i> Tournament Arena List
    </div>
    ...
    <div class="mb-2 d-flex justify-content-end align-items-center">
      ...
      <!-- Create Menu Button -->
      <router-link to="/admin/tournament-arena/create" class="button button-primary">
    ...
    <table class="table mt-4">
      ...
      <tbody v-if="tournamentArenas.length > 0">  
        <tr v-for="(tournamentArena, index) in tournamentArenas" :key="tournamentArena.id">
          <td>{{ index + 1 + (currentPage - 1) * perPage }}</td>
          <td>{{ tournamentArena.tournament.name }}</td>
          <td>{{ tournamentArena.name }}</td>
          ...
          <td class="action-column">
            <div class="btn-group" role="group">
              ...
              <ul class="dropdown-menu">
                <li>
                  <a class="dropdown-item" href="#" @click="EditArena(tournamentArena.id)">
...
export default {
  ...
  data() {
    return {
      tournamentArenas: [], // Array to hold paginated team members
  ...
  mounted() {
    this.loadTournamentArenas(); // Load menu data when the component is mounted
  },
  methods: {
    async loadTournamentArenas(page = 1) {
      this.loading = true;
      try {
        const response = await axios.get("/tournament-arenas", {
          headers: {
        ...
        this.tournamentArenas = data; // Assign team members
    ...
    async EditArena(id) {
      this.$router.push({ name: "EditArena", params: { id } });
    },
