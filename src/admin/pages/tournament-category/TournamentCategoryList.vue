<template>
  <div class="dashboard-container">
    ...
    <div class="mb-2 page-title">
      <i class="bi bi-file-earmark-text"></i> Tournament Category List
    </div>
    ...
    <div class="mb-2 d-flex justify-content-end align-items-center">
      ...
      <!-- Create Menu Button -->
      <router-link to="/admin/tournament-category/create" class="button button-primary">
    ...
    <table class="table mt-4">
      ...
      <tbody v-if="tournamentMatchCategories.length > 0">  
        <tr v-for="(tournamentCategory, index) in tournamentMatchCategories" :key="tournamentCategory.id">
          <td>{{ index + 1 + (currentPage - 1) * perPage }}</td>
          <td>{{ tournamentCategory.tournament.name }}</td>
          <td>{{ tournamentCategory.match_category.name }}</td>
          <td>{{ tournamentCategory.registration_fee }}</td>
          ...
          <td class="action-column">
            <div class="btn-group" role="group">
              ...
              <ul class="dropdown-menu">
                <li>
                  <a class="dropdown-item" href="#" @click="EditCategory(tournamentCategory.id)">
...
export default {
  ...
  data() {
    return {
      tournamentMatchCategories: [], // Array to hold paginated team members
  ...
  mounted() {
    this.loadTournamentMatchCategories(); // Load menu data when the component is mounted
  },
  methods: {
    async loadTournamentMatchCategories(page = 1) {
      this.loading = true;
      try {
        const response = await axios.get("/tournament-match-categories", {
          headers: {
        ...
        this.tournamentMatchCategories = data; // Assign team members
    ...
    changePage(page) {
      if (page >= 1 && page <= this.totalPages) {
        this.loadTournamentMatchCategories(page);
      }
    },
    ...
    searchClasses() {
      ...
      this.loadTournamentMatchCategories();
    },
