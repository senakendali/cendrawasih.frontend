<template>
  <div class="dashboard-container">
    ...
    <div class="mb-2 page-title">
      <i class="bi bi-file-earmark-text"></i> Contact Person List
    </div>
    ...
    <div class="mb-2 d-flex justify-content-end align-items-center">
      ...
      <!-- Create Menu Button -->
      <router-link to="/admin/contact-person/create" class="button button-primary">
    ...
    <table class="table mt-4">
      ...
      <tbody v-if="contactPersons.length > 0">  
        <tr v-for="(contactPerson, index) in contactPersons" :key="contactPerson.id">
          <td>{{ index + 1 + (currentPage - 1) * perPage }}</td>
          <td>{{ contactPerson.tournament.name }}</td>
          <td>{{ contactPerson.description }}</td>
          ...
          <td class="action-column">
            <div class="btn-group" role="group">
              ...
              <ul class="dropdown-menu">
                <li>
                  <a class="dropdown-item" href="#" @click="EditContactPerson(contactPerson.id)">
...
export default {
  ...
  data() {
    return {
      contactPersons: [], // Array to hold paginated team members
  ...
  mounted() {
    this.loadContactPersons(); // Load menu data when the component is mounted
  },
  methods: {
    async loadContactPersons(page = 1) {
      this.loading = true;
      try {
        const response = await axios.get("/tournament-contact-persons", {
          headers: {
        ...
        this.contactPersons = data; // Assign team members
    ...
    async EditContactPerson(id) {
      this.$router.push({ name: "EditContactPerson", params: { id } });
    },
