<template>
  <div class="dashboard-container">
    ...
    <div class="mb-2 page-title">
      <i class="bi bi-file-earmark-text"></i> Tournament Activity List
    </div>
    ...
    <div class="mb-2 d-flex justify-content-end align-items-center">
      ...
      <!-- Create Menu Button -->
      <router-link to="/admin/tournament-activity/create" class="button button-primary">
    ...
    <table class="table mt-4">
      ...
      <tbody v-if="activities.length > 0">
        ...
        <tr v-for="(activity, index) in activities" :key="activity.id">
          ...
          <td>{{ activity.tournament.name }}</td>
          <td>{{ activity.name }}</td>
          ...
          <td class="action-column">
            <div class="btn-group" role="group">
              ...
              <ul class="dropdown-menu">
                <li>
                  <a class="dropdown-item" href="#" @click="EditActivity(activity.id)">
...
export default {
  ...
  data() {
    return {
      activities: [], // Array to hold paginated team members
  ...
  mounted() {
    this.loadActivities(); // Load menu data when the component is mounted
  },
  methods: {
    async loadActivities(page = 1) {
      this.loading = true;
      try {
        const response = await axios.get("/tournament-activities", {
          headers: {
        ...
        this.activities = data; // Assign team members
